...
public class CategoryService(ApplicationDbContext context) : ICategoryService
{
        ...
        List<CategoryTreeDto> BuildSubtree(int? parentId) =>
            [.. childrenByParentId[parentId]
                ...
                .Select(c => new CategoryTreeDto(
                    c.Id, c.Name, c.ImageUrl, c.UrlSlug, c.ParentId, c.SortOrder, TypeName(c.Type), BuildSubtree(c.Id)))];
    ...
    public async Task<CategoryDto> CreateAsync(CreateCategoryDto createDto)
    {
        ...
        {
            var parent = await context.Categories.FirstOrDefaultAsync(c => c.Id == createDto.ParentId);
            if (parent is null)
    ...
    public async Task<CategoryDto?> UpdateAsync(int id, UpdateCategoryDto updateDto)
    {
        ...
        if (updateDto.SortOrder is not null) category.SortOrder = updateDto.SortOrder.Value;
        ...
        if (updateDto.Type is not null)
        {
            ...
            if (newType != category.Type)
            {
                ...
                if (newType == CategoryType.Branch &&
    ...
    private async Task MoveUnderParentAsync(Category category, int newParentId)
    {
        ...
        var allCategories = await context.Categories
            .AsNoTracking()
            .Select(c => new { c.Id, c.ParentId, c.Name, c.Type })
            .ToListAsync();

        var newParent = allCategories.FirstOrDefault(c => c.Id == newParentId);
        if (newParent is null)
            throw new InvalidOperationException($"Parent category with ID {newParentId} does not exist.");
        ...
        while (cursor is not null)
        {
            if (cursor == category.Id)
                throw new InvalidOperationException("Cannot move a category into one of its own descendants.");
            if (++steps > parentById.Count)
                throw new InvalidOperationException("Category hierarchy contains a cycle; fix the data before moving categories.");
    ...
    private static CategoryType ParseType(string value)
    {
        if (!Enum.TryParse<CategoryType>(value, ignoreCase: true, out var type))
            throw new InvalidOperationException($"Invalid category type '{value}'. Valid values: BRANCH, LEAF.");
        return type;
    }
    ...
    public async Task<bool> DeleteAsync(int id)
    {
        ...
        if (hasChildren)
        {
            throw new InvalidOperationException("Cannot delete a category that has subcategories. Delete or move them first.");
        }
