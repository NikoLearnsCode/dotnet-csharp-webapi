...
public class ProductService(ApplicationDbContext context) : IProductService
{
    ...
    )
    {
        ...
        var categoryLookup =
            includeCategories || !string.IsNullOrEmpty(categorySlug)
        ...
        var productDtos = products
            .Select(p => MapToDto(p, includeCategories, categoryLookup, includeImages))
            .ToList();
    ...
    // Cursor pagination
    public async Task<CursorPagedList<ProductDto>> GetAllCursorAsync(
        bool includeCategories,
        int limit,
        string? cursor,
        string? categorySlug,
    ...
    )
    {
        ...
        var productDtos = products
            .Select(p => MapToDto(p, includeCategories, categoryLookup))
            .ToList();

        return new CursorPagedList<ProductDto> { Items = productDtos, NextCursor = nextCursor };
    }
    ...
    public async Task<ProductWithRelatedDto?> GetBySlugAsync(string slug, bool includeCategories)
    {
        ...
        var relatedProducts = await context
    ...
    public async Task<ProductDto> CreateAsync(CreateProductDto createDto)
    {
        ...
        if (nameExists)
            throw new InvalidOperationException(
                $"A product named '{createDto.Name}' already exists."
            );
    ...
    public async Task<ProductDto?> UpdateAsync(int id, UpdateProductDto updateDto)
    {
        ...
        {
            var nameExists = await context.Products.AnyAsync(p =>
                p.Id != id && p.Name.ToLower() == updateDto.Name.ToLower()
            );
            if (nameExists)
                throw new InvalidOperationException(
                    $"A product named '{updateDto.Name}' already exists."
                );
        ...
        if (updateDto.CategoryIds is not null)
        {
            ...
            {
                throw new InvalidOperationException(
                    "A product must belong to at least one category. To keep categories unchanged, omit 'categoryIds' from the request."
                );
            }
    ...
    public async Task<bool> DeleteAsync(int id)
    {
        ...
        var hasOrders = await context.OrderItems.AnyAsync(oi => oi.ProductId == id);
        if (hasOrders)
            throw new InvalidOperationException(
    ...
    private async Task<List<Category>> GetValidatedLeafCategoriesAsync(List<int> categoryIds)
    {
        ...
        if (categories.Count != categoryIds.Count)
        {
            ...
            throw new InvalidOperationException(
                $"The following category IDs were not found: {string.Join(", ", missingCategoryIds)}"
            );
        }
    ...
    private static List<int> GetCategorySubtreeIds(
        string categorySlug,
        IReadOnlyDictionary<int, CategoryNode> lookup
    )
    {
    ...
    private static ProductDto MapToDto(
        Product product,
        bool includeCategories,
        IReadOnlyDictionary<int, CategoryNode>? categoryLookup = null,
    ...
    private static List<CategorySummaryDto> BuildCategoryPath(
        int categoryId,
        IReadOnlyDictionary<int, CategoryNode> lookup
    )
    {
